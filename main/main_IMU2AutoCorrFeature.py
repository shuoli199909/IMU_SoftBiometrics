...
def main_IMU2AutoCorrFeature(Params):
    """Main function of transforming segmented IMU data into auto-correlation features. Save the features into csv files.
    ...
    for i_seg in tqdm.tqdm(range(0, len(df_seg))):
        ...
        feature_autocorr = pd.concat((feature_autocorr, 
    ...
    dir_save = os.path.join(dir_crt, 'data', name_dataset, Params.type_imu, 'feature_autocorr', 'feature_autocorr_con_'+str(Params.condition)+'.csv')
...
if __name__ == "__main__":
    ...
    for type_imu in list_type_imu:
        ...
        for condition in list_condition:
            ...
            main_IMU2AutoCorrFeature(Params=Params)